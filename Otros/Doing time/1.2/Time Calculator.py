...
class time_object():
	def __init__(self, time_string):
		self.hours = int(findall("([0-9]*)h[0-9]*m[0-9]*s", time_string)[0])
		self.minutes = int(findall("[0-9]*h([0-9]*)m[0-9]*s", time_string)[0])
		self.seconds = int(findall("[0-9]*h[0-9]*m([0-9]*)s", time_string)[0])
	...
	def __add__(self, another_object):
		...
		return time_object(f"{hours}h{minutes}m{seconds}s")
...
def polish(raw_entry):
	...
	for word in words:
		if findall("[0-9]*h[0-9]*m[0-9]*s", word):
			objects.append(time_object(word))
...
# Finishing 1.2 (more legible code)
# 26/11/2022 at 7:14 p.m.
